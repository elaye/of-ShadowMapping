...
void main() {
    ...
    avgValue += texture(blurSampler, texCoordVarying * res) * incrementalGaussian.x;
    coefficientSum += incrementalGaussian.x;
    incrementalGaussian.xy *= incrementalGaussian.yz;
    
    // Go through the remaining 8 vertical samples (4 on each side of the center)
    for (float i = 1.0; i <= numBlurPixelsPerSide; i++) { 
        avgValue += texture(blurSampler, texCoordVarying * res - i * blurSize * 
    ...
    // color = texture(blurSampler, texCoordVarying * res);
    // color = vec4(texCoordVarying, 0.0, 1.0);
}